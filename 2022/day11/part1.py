...
class Monkey:
    ...
    @staticmethod
    def parse_test(input: str) -> Callable[[Worry], bool]:
        parts = input.split()
        if parts[0] != "Test:":
    ...
    def inspect_item(self, item: Worry) -> Tuple[Worry, MonkeyId]:
        new_worry = self.op(item) // 3
        new_monkey = self.test_pass if self.test(new_worry) else self.test_fail
        self.num_inspections += 1
        return (new_worry, new_monkey)


def execute_round(monkeys: Dict[MonkeyId, Monkey]):
    for monkey in monkeys.values():
        while monkey.items:
            worry = monkey.items.pop(0)
            new_worry, new_monkey = monkey.inspect_item(worry)
